...
interface Workspace {
  id: string; nama: string; slug: string; created_at: string
}
...
// Halaman yang bisa dikonfigurasi (dengan opsi visitor)
const PAGES_WITH_VISITOR: { key: keyof Permissions; label: string; hasVisitor: boolean }[] = [
...
const G = {
  ...
  modal: { background: 'rgba(7,18,11,0.97)', backdropFilter: 'blur(36px) saturate(150%)', border: '1px solid rgba(255,255,255,0.11)', borderTop: '1px solid rgba(255,255,255,0.2)', borderRadius: 24, width: '100%', maxWidth: 520, boxShadow: '0 32px 80px rgba(0,0,0,0.52)', maxHeight: '90vh', overflowY: 'auto' } as React.CSSProperties,
...
const ACCESS_OPTS = [
  { value: 'none',    label: 'Tidak Ada', desc: 'Tidak tampil di sidebar' },
  { value: 'visitor', label: 'Lihat Saja', desc: 'Bisa lihat, tidak bisa edit' },
  { value: 'full',    label: 'Full',      desc: 'Akses penuh' },
]

function RoleModal({ role, onClose, onSave, workspaceId }: {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ nama: nama.trim(), permissions: perms }),
      })
          ...
          <div>
            ...
            <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 90px', padding: '8px 14px', background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase' }}>Halaman</span>
                {['Tidak Ada', 'Lihat Saja', 'Full'].map((h) => (
                  <span key={h} style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', textAlign: 'center' }}>{h}</span>
                ))}
              </div>
              {PAGES_WITH_VISITOR.map(({ key, label, hasVisitor }, idx) => {
                const current = perms[key]
                const opts = hasVisitor ? ['none','visitor','full'] : ['none','full']
                return (
                  <div key={key} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 90px 90px', padding: '11px 14px', alignItems: 'center', borderBottom: idx < PAGES_WITH_VISITOR.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                    <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.72)', fontWeight: 500 }}>{label}</span>
                    {/* Tidak Ada */}
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      <button onClick={() => setAccess(key, 'none')} style={{ width: 22, height: 22, borderRadius: '50%', border: current === 'none' ? '2px solid #ef4444' : '2px solid rgba(255,255,255,0.15)', background: current === 'none' ? 'rgba(239,68,68,0.2)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {current === 'none' && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#ef4444' }} />}
                      </button>
                    </div>
                    {/* Lihat Saja */}
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      {hasVisitor ? (
                        <button onClick={() => setAccess(key, 'visitor')} style={{ width: 22, height: 22, borderRadius: '50%', border: current === 'visitor' ? '2px solid #f59e0b' : '2px solid rgba(255,255,255,0.15)', background: current === 'visitor' ? 'rgba(245,158,11,0.2)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {current === 'visitor' && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#f59e0b' }} />}
                        </button>
                      ) : (
                    ...
                    {/* Full */}
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      <button onClick={() => setAccess(key, 'full')} style={{ width: 22, height: 22, borderRadius: '50%', border: current === 'full' ? '2px solid #10b981' : '2px solid rgba(255,255,255,0.15)', background: current === 'full' ? 'rgba(16,185,129,0.2)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {current === 'full' && <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#10b981' }} />}
                      </button>
                    </div>
                  </div>
                )
            ...
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginTop: 8, lineHeight: 1.6 }}>
              <span style={{ color: '#ef444499' }}>● Tidak Ada</span> — halaman tersembunyi dari sidebar · <span style={{ color: '#f59e0b99' }}>● Lihat Saja</span> — bisa masuk tapi tidak bisa ubah data · <span style={{ color: '#10b98199' }}>● Full</span> — akses penuh
            </p>
          </div>
        </div>
    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto animate-slide-up">
      ...
      {tab === 'roles' && (
        <div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.09)', borderTop: '1px solid rgba(255,255,255,0.14)', borderRadius: '1.125rem', overflow: 'hidden' }}>
            {roles.length === 0 && (
                ...
                <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 22px', borderBottom: idx < roles.length - 1 ? '1px solid rgba(255,255,255,0.045)' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.28)', marginTop: 2 }}>
                      {fullCount} full · {visitorCount} lihat saja
                      {r.is_super_admin && ' · Role sistem'}
                    </p>
                  </div>
                  {!r.is_super_admin && (
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button onClick={() => { setModalRole(r); setShowModal(true) }} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center', gap: 5, fontSize: 12 }}>
                        <Edit2 size={12} /> Edit
                      </button>
                      <button onClick={() => handleDelete(r)} disabled={deleting === r.id} style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.18)', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: '#fca5a5', display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, opacity: deleting === r.id ? 0.5 : 1 }}>
