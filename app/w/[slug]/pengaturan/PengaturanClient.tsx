...
import { Building2, Plus, Trash2, Edit2, X, ShieldCheck, Truck, Pencil } from 'lucide-react'
...
// Dot radio button untuk tabel permissions
...
// ── PengaturanClient ───────────────────────────────────────────────────────────
export default function PengaturanClient({ workspace, roles: initialRoles, kurirList: initialKurirList, slug }: {
  workspace: Workspace; roles: WorkspaceRole[]; kurirList: Kurir[]; slug: string
}) {
  const [tab, setTab]             = useState<'info'|'roles'|'kurir'>('info')
  ...
  const [showKurirModal, setShowKurirModal] = useState(false)
  ...
  // ── Kurir CRUD ───────────────────────────────────────────────────────────────
  function openKurirModal(k?: Kurir) {
    setEditKurir(k ?? null)
    setKurirNama(k?.nama ?? '')
    setKurirHp(k?.no_hp ?? '')
  ...
  return (
    <div className="p-6 md:p-8 pb-20 md:pb-8 max-w-3xl mx-auto animate-slide-up">
          ...
          <div style={G.card}>
            {kurirList.length === 0 && (
              <div style={{ padding:'48px 0', textAlign:'center' }}>
                ...
                <p style={{ fontSize:13, color:'rgba(255,255,255,0.28)', margin:0 }}>Belum ada kurir — klik Tambah Kurir untuk memulai</p>
              </div>
            )}
            {kurirList.map((k, idx) => (
              <div key={k.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 20px', borderBottom: idx < kurirList.length-1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                <div style={{ width:36, height:36, borderRadius:10, background:'rgba(16,185,129,0.08)', border:'1px solid rgba(16,185,129,0.15)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <Truck size={15} color="#34d399" />
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ fontSize:13.5, fontWeight:600, color:'rgba(255,255,255,0.88)', margin:0 }}>{k.nama}</p>
                  {k.no_hp && <p style={{ fontSize:12, color:'rgba(255,255,255,0.35)', margin:'2px 0 0' }}>{k.no_hp}</p>}
                </div>
                <div style={{ display:'flex', gap:6 }}>
                  ...
                  <button onClick={() => handleDeleteKurir(k.id, k.nama)} disabled={deletingKurir === k.id}
                    style={{ background:'rgba(239,68,68,0.07)', border:'1px solid rgba(239,68,68,0.15)', borderRadius:8, padding:'6px 10px', cursor:'pointer', color:'#fca5a5', display:'flex', alignItems:'center', gap:5, fontSize:12, opacity: deletingKurir===k.id ? 0.5 : 1 }}>
                    <Trash2 size={12} /> Hapus
                  </button>
                </div>
              </div>
      ...
      {/* Modal Role */}
      {showModal && (
        <RoleModal role={modalRole} onClose={() => setShowModal(false)} onSave={handleSaved} />
      )}

      {/* Modal Tambah/Edit Kurir */}
      {showKurirModal && (
        <ModalPortal>
          <div style={G.overlay} onClick={(e) => { if (e.target === e.currentTarget) setShowKurirModal(false) }}>
            <div style={{ ...G.modal, maxWidth:400, margin:'auto' }}>
              <div style={{ padding:'20px 24px 16px', borderBottom:'1px solid rgba(255,255,255,0.08)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <h2 style={{ fontSize:15, fontWeight:800, color:'rgba(255,255,255,0.95)', margin:0 }}>
                  {editKurir ? 'Edit Kurir' : 'Tambah Kurir'}
                </h2>
                <button onClick={() => setShowKurirModal(false)} style={{ background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, width:32, height:32, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(255,255,255,0.42)' }}>
                  <X size={14} />
                </button>
              </div>
              <div style={{ padding:'20px 24px', display:'flex', flexDirection:'column', gap:14 }}>
                ...
                <div>
                  <label style={{ display:'block', fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.36)', letterSpacing:'0.8px', textTransform:'uppercase', marginBottom:7 }}>No. HP <span style={{ fontWeight:400, textTransform:'none', opacity:0.5 }}>(opsional)</span></label>
