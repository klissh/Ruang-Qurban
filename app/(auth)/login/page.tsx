      ...
      <div className="w-full max-w-[400px] relative z-10">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex relative mb-5">
            <div className="w-[74px] h-[74px] rounded-3xl flex items-center justify-center" style={{ background: 'linear-gradient(145deg, #12c98d 0%, #059669 100%)', boxShadow: '0 0 0 1px rgba(16,185,129,0.28), 0 8px 40px rgba(16,185,129,0.42), 0 0 80px rgba(16,185,129,0.12)' }}>
              <Moon size={30} color="white" strokeWidth={2.2} />
            </div>
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight" style={{ color: 'rgba(255,255,255,0.97)', letterSpacing: '-0.5px' }}>Portal Qurban</h1>
          <p className="text-sm mt-2" style={{ color: 'rgba(255,255,255,0.35)' }}>Sistem Manajemen Kepanitiaan Qurban</p>
        </div>

        {/* Card */}
        <div className="rounded-3xl p-8" style={{ background: 'rgba(255,255,255,0.07)', backdropFilter: 'blur(32px) saturate(150%)', WebkitBackdropFilter: 'blur(32px) saturate(150%)', border: '1px solid rgba(255,255,255,0.1)', borderTop: '1px solid rgba(255,255,255,0.2)', boxShadow: '0 24px 64px rgba(0,0,0,0.38), inset 0 1px 0 rgba(255,255,255,0.09)' }}>
          <p className="text-xs font-bold uppercase tracking-widest mb-6" style={{ color: 'rgba(255,255,255,0.38)', letterSpacing: '0.8px' }}>Masuk ke Akun Anda</p>

          {error && (
            <div className="text-sm rounded-xl px-4 py-3 mb-5" style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', color: '#fca5a5' }}>{error}</div>
          )}
          ...
          <div className="mb-4">
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleLogin()} placeholder="panitia@masjid.com"
          ...
          <button onClick={handleLogin} disabled={loading} className="w-full rounded-2xl py-3.5 text-sm font-bold text-white transition-all disabled:opacity-60"
